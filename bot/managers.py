...
class UserManager:
    ...
    async def register_user(
        self, user: User, client: httpx.AsyncClient
    ) -> httpx.Response:
    ...
    async def obtain_jwt(
        ...
        async with self.semaphore:
            response = await client.post(
                urls.OBTAIN_JWT_URL, data=user.__dict__
            )
...
class PostManager:
    ...
    async def create_post(
        self, client: httpx.AsyncClient, post: Post, headers: dict
    ) -> httpx.Response:
        async with self.semaphore:
            response = await client.post(
    ...
    async def like_post(
        self, client: httpx.AsyncClient, post_id: int, headers: dict
    ) -> httpx.Response:
